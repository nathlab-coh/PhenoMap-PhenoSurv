...
```{r}
br_pca_df_predicted$PatientID <- br_her2_predicted$PatientID
pi3k <- tcga_genomic[,c("PatientID", "SNV_PIK3CA")]
br_pca_df_predicted <- merge(br_pca_df_predicted, pi3k, by = "PatientID")
br_pca_df_predicted$`PI3KCA Mutation Status` = "No PIK3CA Mutation"
br_pca_df_predicted$`PI3KCA Mutation Status`[br_pca_df_predicted$SNV_PIK3CA == 1] <- "PIK3CA Mutation"

br_pca_df_predicted <- br_pca_df_predicted[order(br_pca_df_predicted$`PI3KCA Mutation Status` == "PIK3CA Mutation"),]

pi3k_pm <- ggplot(br_pca_df_predicted, aes(x = PC1, y = PC2, color = `PI3KCA Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
                  scale_color_manual(values = c("PIK3CA Mutation" = "#1E88E5", "No PIK3CA Mutation" = "lightgrey"))
pi3k_pm
```
```{r}
br_pca_df_real$PatientID <- br_her2_real$PatientID
br_pca_df_real <- merge(br_pca_df_real, pi3k, by = "PatientID")
br_pca_df_real$`PI3KCA Mutation Status` = "No PIK3CA Mutation"
br_pca_df_real$`PI3KCA Mutation Status`[br_pca_df_real$SNV_PIK3CA == 1] <- "PIK3CA Mutation"

br_pca_df_real <- br_pca_df_real[order(br_pca_df_real$`PI3KCA Mutation Status` == "PIK3CA Mutation"),]
pi3k_gsva <- ggplot(br_pca_df_real, aes(x = PC1, y = PC2, color = `PI3KCA Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
egfr <- tcga_genomic[,c("PatientID", "SNV_EGFR")]
br_pca_df_predicted <- merge(br_pca_df_predicted, egfr, by = "PatientID")
br_pca_df_predicted$`EGFR Mutation Status` = "No EGFR Mutation"
br_pca_df_predicted$`EGFR Mutation Status`[br_pca_df_predicted$SNV_EGFR == 1] <- "EGFR Mutation"

br_pca_df_predicted <- br_pca_df_predicted[order(br_pca_df_predicted$`EGFR Mutation Status` == "EGFR Mutation"),]
egfr_pm <- ggplot(br_pca_df_predicted, aes(x = PC1, y = PC2, color = `EGFR Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
                  scale_color_manual(values = c("EGFR Mutation" = "#FFC107", "No EGFR Mutation" = "lightgrey"))
egfr_pm
```
```{r}
br_pca_df_real$PatientID <- br_her2_real$PatientID
br_pca_df_real <- merge(br_pca_df_real, egfr, by = "PatientID")
br_pca_df_real$`EGFR Mutation Status` = "No EGFR Mutation"
br_pca_df_real$`EGFR Mutation Status`[br_pca_df_real$SNV_EGFR == 1] <- "EGFR Mutation"

br_pca_df_real <- br_pca_df_real[order(br_pca_df_real$`EGFR Mutation Status` == "EGFR Mutation"),]
egfr_gsva <- ggplot(br_pca_df_real, aes(x = PC1, y = PC2, color = `EGFR Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
cdk4 <- tcga_genomic[,c("PatientID", "SNV_CDK4", "SNV_CDK6")]
br_pca_df_predicted <- merge(br_pca_df_predicted, cdk4, by = "PatientID")
br_pca_df_predicted$`CDK4/6 Mutation Status` = "No CDK4/6 Mutation"
br_pca_df_predicted$`CDK4/6 Mutation Status`[br_pca_df_predicted$SNV_CDK4 == 1] <- "CDK4/6 Mutation"
br_pca_df_predicted$`CDK4/6 Mutation Status`[br_pca_df_predicted$SNV_CDK6 == 1] <- "CDK4/6 Mutation"

br_pca_df_predicted <- br_pca_df_predicted[order(br_pca_df_predicted$`CDK4/6 Mutation Status` == "CDK4/6 Mutation"),]
cdk_pm <- ggplot(br_pca_df_predicted, aes(x = PC1, y = PC2, color = `CDK4/6 Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
                  scale_color_manual(values = c("CDK4/6 Mutation" = "#D81B60", "No CDK4/6 Mutation" = "lightgrey"))
cdk_pm
```
```{r}
br_pca_df_real$PatientID <- br_her2_real$PatientID
br_pca_df_real <- merge(br_pca_df_real, cdk4, by = "PatientID")
br_pca_df_real$`CDK4/6 Mutation Status` = "No CDK4/6 Mutation"
br_pca_df_real$`CDK4/6 Mutation Status`[br_pca_df_real$SNV_CDK4 == 1] <- "CDK4/6 Mutation"
br_pca_df_real$`CDK4/6 Mutation Status`[br_pca_df_real$SNV_CDK6 == 1] <- "CDK4/6 Mutation"

br_pca_df_real <- br_pca_df_real[order(br_pca_df_real$`CDK4/6 Mutation Status` == "CDK4/6 Mutation"),]
cdk_gsva <- ggplot(br_pca_df_real, aes(x = PC1, y = PC2, color = `CDK4/6 Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
Correlation plots
```{r}
pi3k_cor_df_real <- br_her2_real[,c("PatientID", "REACTOME_PI3K_AKT_ACTIVATION")]
colnames(pi3k_cor_df_real)[2] <- "GSVA Scores"
pi3k_cor_df_pred <- br_her2_predicted[,c("PatientID", "REACTOME_PI3K_AKT_ACTIVATION")]
colnames(pi3k_cor_df_pred)[2] <- "PhenoMap Scores"

pi3k_cor_df <- merge(pi3k_cor_df_pred, pi3k_cor_df_real, by = "PatientID")

pi3k_cor <- ggplot(pi3k_cor_df, aes(x = `GSVA Scores`, y = `PhenoMap Scores`)) +
                  geom_point(color = "#1E88E5", size = 1) +
                  theme_classic() +
                  ggtitle("PI3K Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
                  stat_cor(method = "spearman") +
                  geom_smooth(method = "lm", color = "#1E88E5")
pi3k_cor
```

```{r}
egfr_cor_df_real <- br_her2_real[,c("PatientID", "REACTOME_SIGNALING_BY_EGFR_IN_CANCER")]
colnames(egfr_cor_df_real)[2] <- "GSVA Scores"
egfr_cor_df_pred <- br_her2_predicted[,c("PatientID", "REACTOME_SIGNALING_BY_EGFR_IN_CANCER")]
colnames(egfr_cor_df_pred)[2] <- "PhenoMap Scores"

egfr_cor_df <- merge(egfr_cor_df_pred, egfr_cor_df_real, by = "PatientID")

egfr_cor <- ggplot(egfr_cor_df, aes(x = `GSVA Scores`, y = `PhenoMap Scores`)) +
                  geom_point(color = "#FFC107", size = 1) +
                  theme_classic() +
                  ggtitle("EGFR Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
                  stat_cor(method = "spearman") +
                  geom_smooth(method = "lm", color = "#FFC107")
egfr_cor
```

```{r}
cdk_cor_df_real <- br_her2_real[,c("PatientID", "REACTOME_G1_S_SPECIFIC_TRANSCRIPTION")]
colnames(cdk_cor_df_real)[2] <- "GSVA Scores"
cdk_cor_df_pred <- br_her2_predicted[,c("PatientID", "REACTOME_G1_S_SPECIFIC_TRANSCRIPTION")]
colnames(cdk_cor_df_pred)[2] <- "PhenoMap Scores"

cdk_cor_df <- merge(cdk_cor_df_pred, cdk_cor_df_real, by = "PatientID")

cdk_cor <- ggplot(cdk_cor_df, aes(x = `GSVA Scores`, y = `PhenoMap Scores`)) +
                  geom_point(color = "#D81B60", size = 1) +
                  theme_classic() +
                  ggtitle("CDK4/6 Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
                  stat_cor(method = "spearman") +
                  geom_smooth(method = "lm", color = "#D81B60")
cdk_cor
```

Split violin plots GSVA vs PhenoMap scores w/wo mutation
```{r}
pi3k_cor_df <- merge(pi3k_cor_df, br_pca_df_predicted[,c("PatientID", "PI3KCA Mutation Status", "Group")], by = "PatientID")
```
...
pi3k_plot <- ggplot(pi3k_cor_df, aes(x = Group, y = `PhenoMap Scores`, fill = `PI3KCA Mutation Status`)) +
  introdataviz::geom_split_violin(alpha = 1, trim = FALSE) +
  geom_boxplot(width = .2, alpha = 0.2, show.legend = T, outliers = F, color = "white") +
  scale_x_discrete(name = "Group", labels = c("EGFR Signaling High", "CDK4/6 Signaling High", "PI3K Signaling High")) +
  scale_fill_manual(values = c("No PIK3CA Mutation" = "#1E88E5", "PIK3CA Mutation" = "#685CAF")) +
  theme_classic()+
...
```{r}
egfr_cor_df <- merge(egfr_cor_df, br_pca_df_predicted[,c("PatientID", "EGFR Mutation Status", "Group")], by = "PatientID")
```
...
egfr_plot <- ggplot(egfr_cor_df, aes(x = Group, y = `PhenoMap Scores`, fill = `EGFR Mutation Status`)) +
  introdataviz::geom_split_violin(alpha = 1, trim = FALSE) +
  geom_boxplot(width = .1, alpha = 0.2, show.legend = T, outliers = F, color = "white") +
  scale_x_discrete(name = "Group", labels = c("EGFR Signaling High", "CDK4/6 Signaling High", "PI3K Signaling High")) +
  scale_fill_manual(values = c("No EGFR Mutation" = "#FFC107", "EGFR Mutation" = "#4B93B8")) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "right")
egfr_plot
```
...
```{r}
cdk_cor_df <- merge(cdk_cor_df, br_pca_df_predicted[,c("PatientID", "CDK4/6 Mutation Status", "Group")], by = "PatientID")
```
...
cdk_plot <- ggplot(cdk_cor_df, aes(x = Group, y = `PhenoMap Scores`, fill = `CDK4/6 Mutation Status`)) +
  introdataviz::geom_split_violin(alpha = 1, trim = FALSE) +
  geom_boxplot(width = .1, alpha = 0.2, show.legend = T, outliers = F, color = "white") +
  scale_x_discrete(name = "Group", labels = c("EGFR Signaling High", "CDK4/6 Signaling High", "PI3K Signaling High")) +
  scale_fill_manual(values = c("No CDK4/6 Mutation" = "#D81B60", "CDK4/6 Mutation" = "#8D4695")) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "right")
cdk_plot
```
...
```{r}
ggarrange(pi3k_plot, egfr_plot, cdk_plot, ncol = 1, nrow = 3)
```
```{r}
ggarrange(pi3k_cor, egfr_cor, cdk_cor, ncol = 3, nrow = 1)
```

```{r}
ggpubr::ggarrange(gsva_pca, pm_pca, 
                  pi3k_gsva, pi3k_pm,
                  egfr_gsva, egfr_pm,
                  cdk_gsva, cdk_pm,
...
gsva_pca_lu <- ggplot(lu_pca_df_real, aes(x = PC1, y = PC2, color = Group)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
pm_pca_lu <- ggplot(lu_pca_df_predicted, aes(x = PC1, y = PC2, color = Group)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```
```{r}
lu_pca_df_predicted$PatientID <- lu_her2_predicted$PatientID
pi3k <- tcga_genomic[,c("PatientID", "SNV_PIK3CA")]
lu_pca_df_predicted <- merge(lu_pca_df_predicted, pi3k, by = "PatientID")
lu_pca_df_predicted$`PI3KCA Mutation Status` = "No PIK3CA Mutation"
lu_pca_df_predicted$`PI3KCA Mutation Status`[lu_pca_df_predicted$SNV_PIK3CA == 1] <- "PIK3CA Mutation"

lu_pca_df_predicted <- lu_pca_df_predicted[order(lu_pca_df_predicted$`PI3KCA Mutation Status` == "PIK3CA Mutation"),]
pi3k_pm_lu <- ggplot(lu_pca_df_predicted, aes(x = PC1, y = PC2, color = `PI3KCA Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```
```{r}
lu_pca_df_real$PatientID <- lu_her2_real$PatientID
lu_pca_df_real <- merge(lu_pca_df_real, pi3k, by = "PatientID")
lu_pca_df_real$`PI3KCA Mutation Status` = "No PIK3CA Mutation"
lu_pca_df_real$`PI3KCA Mutation Status`[lu_pca_df_real$SNV_PIK3CA == 1] <- "PIK3CA Mutation"

lu_pca_df_real <- lu_pca_df_real[order(lu_pca_df_real$`PI3KCA Mutation Status` == "PIK3CA Mutation"),]
pi3k_gsva_lu <- ggplot(lu_pca_df_real, aes(x = PC1, y = PC2, color = `PI3KCA Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
egfr <- tcga_genomic[,c("PatientID", "SNV_EGFR")]
lu_pca_df_predicted <- merge(lu_pca_df_predicted, egfr, by = "PatientID")
lu_pca_df_predicted$`EGFR Mutation Status` = "No EGFR Mutation"
lu_pca_df_predicted$`EGFR Mutation Status`[lu_pca_df_predicted$SNV_EGFR == 1] <- "EGFR Mutation"

lu_pca_df_predicted <- lu_pca_df_predicted[order(lu_pca_df_predicted$`EGFR Mutation Status` == "EGFR Mutation"),]
egfr_pm_lu <- ggplot(lu_pca_df_predicted, aes(x = PC1, y = PC2, color = `EGFR Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
lu_pca_df_real$PatientID <- lu_her2_real$PatientID
lu_pca_df_real <- merge(lu_pca_df_real, egfr, by = "PatientID")
lu_pca_df_real$`EGFR Mutation Status` = "No EGFR Mutation"
lu_pca_df_real$`EGFR Mutation Status`[lu_pca_df_real$SNV_EGFR == 1] <- "EGFR Mutation"

lu_pca_df_real <- lu_pca_df_real[order(lu_pca_df_real$`EGFR Mutation Status` == "EGFR Mutation"),]
egfr_gsva_lu <- ggplot(lu_pca_df_real, aes(x = PC1, y = PC2, color = `EGFR Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
kras <- tcga_genomic[,c("PatientID", "SNV_KRAS")]
lu_pca_df_predicted <- merge(lu_pca_df_predicted, kras, by = "PatientID")
lu_pca_df_predicted$`KRAS Mutation Status` = "No KRAS Mutation"
lu_pca_df_predicted$`KRAS Mutation Status`[lu_pca_df_predicted$SNV_KRAS == 1] <- "KRAS Mutation"

lu_pca_df_predicted <- lu_pca_df_predicted[order(lu_pca_df_predicted$`KRAS Mutation Status` == "KRAS Mutation"),]
kras_pm <- ggplot(lu_pca_df_predicted, aes(x = PC1, y = PC2, color = `KRAS Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("PhenoMap") +
                  theme(plot.title = element_text(hjust = 0.5))+
                  scale_color_manual(values = c("KRAS Mutation" = "#789E8C", "No KRAS Mutation" = "lightgrey"))
kras_pm
```
```{r}
kras <- tcga_genomic[,c("PatientID", "SNV_KRAS")]
lu_pca_df_real <- merge(lu_pca_df_real, kras, by = "PatientID")
lu_pca_df_real$`KRAS Mutation Status` = "No KRAS Mutation"
lu_pca_df_real$`KRAS Mutation Status`[lu_pca_df_real$SNV_KRAS == 1] <- "KRAS Mutation"

lu_pca_df_real <- lu_pca_df_real[order(lu_pca_df_real$`KRAS Mutation Status` == "KRAS Mutation"),]
kras_gsva <- ggplot(lu_pca_df_real, aes(x = PC1, y = PC2, color = `KRAS Mutation Status`)) +
                  geom_point(size = 1) +
                  theme_classic() +
                  ggtitle("GSVA") +
                  theme(plot.title = element_text(hjust = 0.5))+
...
```{r}
ggarrange(pi3k_gsva, egfr_gsva, cdk_gsva, ncol = 1, nrow = 3, align = "hv")
```

```{r}
ggarrange(pi3k_gsva_lu, egfr_gsva_lu, kras_gsva, ncol = 1, nrow = 3, align = "hv")
```

Correlation plots
```{r}
pi3k_cor_df_real <- lu_her2_real[,c("PatientID", "REACTOME_PI3K_AKT_ACTIVATION")]
colnames(pi3k_cor_df_real)[2] <- "GSVA Scores"
pi3k_cor_df_pred <- lu_her2_predicted[,c("PatientID", "REACTOME_PI3K_AKT_ACTIVATION")]
colnames(pi3k_cor_df_pred)[2] <- "PhenoMap Scores"

pi3k_cor_df <- merge(pi3k_cor_df_pred, pi3k_cor_df_real, by = "PatientID")

pi3k_cor_lu <- ggplot(pi3k_cor_df, aes(x = `GSVA Scores`, y = `PhenoMap Scores`)) +
                  geom_point(color = "#1E88E5", size = 1) +
                  theme_classic() +
                  ggtitle("PI3K Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
...
```{r}
egfr_cor_df_real <- lu_her2_real[,c("PatientID", "REACTOME_SIGNALING_BY_EGFR_IN_CANCER")]
colnames(egfr_cor_df_real)[2] <- "GSVA Scores"
egfr_cor_df_pred <- lu_her2_predicted[,c("PatientID", "REACTOME_SIGNALING_BY_EGFR_IN_CANCER")]
colnames(egfr_cor_df_pred)[2] <- "PhenoMap Scores"

egfr_cor_df <- merge(egfr_cor_df_pred, egfr_cor_df_real, by = "PatientID")

egfr_cor_lu <- ggplot(egfr_cor_df, aes(x = `GSVA Scores`, y = `PhenoMap Scores`)) +
                  geom_point(color = "#FFC107", size = 1) +
                  theme_classic() +
                  ggtitle("EGFR Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
...
```{r}
kras_cor_df_real <- lu_her2_real[,c("PatientID", "HALLMARK_KRAS_SIGNALING_UP")]
colnames(kras_cor_df_real)[2] <- "GSVA Scores"
kras_cor_df_pred <- lu_her2_predicted[,c("PatientID", "HALLMARK_KRAS_SIGNALING_UP")]
colnames(kras_cor_df_pred)[2] <- "PhenoMap Scores"

kras_cor_df <- merge(kras_cor_df_pred, kras_cor_df_real, by = "PatientID")

kras_cor <- ggplot(kras_cor_df, aes(x = `GSVA Scores`, y = `PhenoMap Scores`)) +
                  geom_point(color = "#789E8C", size = 1) +
                  theme_classic() +
                  ggtitle("KRAS Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
                  stat_cor(method = "spearman") +
                  geom_smooth(method = "lm", color = "#789E8C")
kras_cor
```

Split violin plots GSVA vs PhenoMap scores w/wo mutation
```{r}
pi3k_cor_df <- merge(pi3k_cor_df, lu_pca_df_predicted[,c("PatientID", "PI3KCA Mutation Status", "Group")], by = "PatientID")
```
...
pi3k_plot_lu <- ggplot(pi3k_cor_df, aes(x = Group, y = `PhenoMap Scores`, fill = `PI3KCA Mutation Status`)) +
  introdataviz::geom_split_violin(alpha = 1, trim = FALSE) +
  geom_boxplot(width = .2, alpha = 0.2, show.legend = T, outliers = F, color = "white") +
  scale_x_discrete(name = "Group", labels = c("EGFR Signaling High", "KRAS Signaling High", "PI3K Signaling High")) +
  scale_fill_manual(values = c("No PIK3CA Mutation" = "#1E88E5", "PIK3CA Mutation" = "#685CAF")) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "right")
pi3k_plot_lu
...
```{r}
egfr_cor_df <- merge(egfr_cor_df, lu_pca_df_predicted[,c("PatientID", "EGFR Mutation Status", "Group")], by = "PatientID")
```
...
egfr_plot_lu <- ggplot(egfr_cor_df, aes(x = Group, y = `PhenoMap Scores`, fill = `EGFR Mutation Status`)) +
  introdataviz::geom_split_violin(alpha = 1, trim = FALSE) +
  geom_boxplot(width = .1, alpha = 0.2, show.legend = T, outliers = F, color = "white") +
  scale_x_discrete(name = "Group", labels = c("EGFR Signaling High", "KRAS Signaling High", "PI3K Signaling High")) +
  scale_fill_manual(values = c("No EGFR Mutation" = "#FFC107", "EGFR Mutation" = "#B2307A")) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "right")
egfr_plot_lu
```
...
```{r}
kras_cor_df <- merge(kras_cor_df, lu_pca_df_predicted[,c("PatientID", "KRAS Mutation Status", "Group")], by = "PatientID")
```
...
kras_plot_lu <- ggplot(kras_cor_df, aes(x = Group, y = `PhenoMap Scores`, fill = `KRAS Mutation Status`)) +
  introdataviz::geom_split_violin(alpha = 1, trim = FALSE) +
  geom_boxplot(width = .1, alpha = 0.2, show.legend = T, outliers = F, color = "white") +
  scale_x_discrete(name = "Group", labels = c("EGFR Signaling High", "KRAS Signaling High", "PI3K Signaling High")) +
  scale_fill_manual(values = c("No KRAS Mutation" = "#789E8C", "KRAS Mutation" = "#4372CA")) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "right")
kras_plot_lu
```
...
```{r}
ggarrange(pi3k_plot_lu, egfr_plot_lu, kras_plot_lu, ncol = 1, nrow = 3)
```
```{r}
ggarrange(pi3k_cor_lu, egfr_cor_lu, kras_cor, ncol = 3, nrow = 1)
```

```{r}
ggpubr::ggarrange(pm_pca_lu, 
                  pi3k_pm_lu,
                  egfr_pm_lu,
                  kras_pm,
...
g1_plot <- ggplot(g1_s_imp[1:20,], aes(x = Gene, y = rev(IncNodePurity))) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 25), axis.text = element_text(size = 18), axis.title = element_text(size = 20))
...
```{r}
ggpubr::ggarrange(g1_plot, egfr_plot, pi3k_plot, kras_plot, ncol = 4, nrow = 1)
```

```{r}
clin <- read.csv("~//Documents//Survival VAE//PANCAN_All_clinical.csv")
clin <- clin[,c("bcr_patient_barcode", "tobacco_smoking_history", "number_pack_years_smoked", "gender")]
colnames(clin)[[1]] <- "PatientID"

lu_clin <- merge(lu_pca_df_predicted, clin, by = "PatientID")
lu_clin2 <- merge(kras_cor_df, clin, by = "PatientID")
```

```{r}
lu_clin <- subset(lu_clin, tobacco_smoking_history != "[Not Available]" & tobacco_smoking_history != "[Unknown]")
lu_clin$tobacco_smoking_history[lu_clin$tobacco_smoking_history %in% c("Current reformed smoker for < or = 15 years",
                                                                       "Current reformed smoker for > 15 years",
                                                                       "Current Reformed Smoker, Duration Not Specified")] <- "Reformed Smoker"
lu_clin$tobacco_smoking_history[lu_clin$tobacco_smoking_history %in% c("Lifelong Non-smoker")] <- "Non-Smoker"
lu_clin$tobacco_smoking_history[lu_clin$tobacco_smoking_history %in% c("Current smoker")] <- "Current Smoker"

df2 <- data.frame()
for (sm in unique(lu_clin$tobacco_smoking_history)){
  rows <- data.frame(table(subset(lu_clin, tobacco_smoking_history == sm)$Group))
  rows$`Smoking Status` = sm
  colnames(rows)[1:2] <- c("Group", "Frequency")
  df2 <- rbind(df2, rows)
}

df3 <- data.frame()
for (sm in unique(lu_clin$gender)){
  rows <- data.frame(table(subset(lu_clin, gender == sm)$Group))
  rows$`Gender` = sm
  colnames(rows)[1:2] <- c("Group", "Frequency")
  df3 <- rbind(df3, rows)
}
...
```{r}
lu_clin2 <- subset(lu_clin2, number_pack_years_smoked != "" & number_pack_years_smoked != "[Not Available]")
lu_clin2$number_pack_years_smoked <- as.numeric(lu_clin2$number_pack_years_smoked)

smoke_cor <- ggplot(lu_clin2, aes(x = number_pack_years_smoked, y = `PhenoMap Scores`)) +
                  geom_point(color = "#789E8C", size = 1) +
                  theme_classic() +
                  #ggtitle("KRAS Signaling") +
                  theme(plot.title = element_text(hjust = 0.5))+ 
...
```{r}
group_plot <- ggplot(df2, aes(x = `Smoking Status`, y = Frequency, fill = Group)) +
  geom_bar(position = "fill", stat = "identity", color = "black") +
  theme_classic() +
  ylab(NULL) +
  scale_fill_manual(values = c("EGFR Signaling High" = "#FFC107", "PI3K Signaling High" = "#1E88E5", 
                               "KRAS Signaling High" = "#789E8C")) +
  geom_text(aes(label = Frequency), size = 6, position = position_fill(vjust = 0.5)) +
  theme(axis.text = element_text(size = 16), axis.title = element_text(size = 16), legend.text = element_text(size = 14),
        legend.title = element_text(size = 14), axis.text.y = element_blank(), axis.title.y = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.ticks.y = element_blank())
group_plot
```

```{r}
clin <- read.csv("~//Documents//Survival VAE//breast_clin_top_0.25.csv")
clin <- clin[,c("PATIENT_ID", "SUBTYPE")]
colnames(clin)[[1]] <- "PatientID"

br_clin <- merge(br_pca_df_predicted, clin, by = "PatientID")
```

```{r}
br_clin <- subset(br_clin, SUBTYPE != "BRCA_Normal" & SUBTYPE != "")
df2 <- data.frame()
for (sm in unique(br_clin$SUBTYPE)){
  rows <- data.frame(table(subset(br_clin, SUBTYPE == sm)$Group))
  rows$`Subtype` = sm
  colnames(rows)[1:2] <- c("Group", "Frequency")
  df2 <- rbind(df2, rows)
}
...
```{r}
group_plot <- ggplot(df2, aes(x = `Subtype`, y = Frequency, fill = Group)) +
  geom_bar(position = "fill", stat = "identity", color = "black") +
  theme_classic() +
  ylab(NULL) +
  scale_fill_manual(values = c("EGFR Signaling High" = "#FFC107", "PI3K Signaling High" = "#1E88E5", 
                               "CDK4/6 Signaling High" = "#D81B60")) +
  #geom_text(aes(label = Frequency), size = 6, position = position_fill(vjust = 0.5)) +
  theme(axis.text = element_text(size = 16), axis.title = element_text(size = 16), legend.text = element_text(size = 14),
        legend.title = element_text(size = 14), axis.text.y = element_blank(), axis.title.y = element_blank(),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), axis.ticks.y = element_blank(),
